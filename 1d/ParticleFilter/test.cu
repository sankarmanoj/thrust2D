...
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    thrust::host_vector<double> CDF(Nparticles);                //not so important
    thrust::host_vector<double> u(Nparticles);
    thrust::host_vector<double> likelihood(Nparticles);
    thrust::host_vector<int> ind(countOnes * Nparticles);    
	thrust::host_vector<int> seed_h(Nparticles);

    thrust::fill(xj.begin(), xj.end(), xe);
    thrust::fill(yj.begin(), yj.end(), ye);
    
    thrust::device_vector<double> arrayX_device(Nparticles);
    thrust::device_vector<double> arrayY_device(Nparticles);
    thrust::device_vector<double> CDF_device(Nparticles);
    thrust::device_vector<double> u_device(Nparticles);    
    thrust::device_vector<double> likelihood_device(Nparticles, 0);
    thrust::device_vector<double> weights_device(Nparticles);
    thrust::device_vector<double> partial_sums_device(Nparticles);         //added this for compatibility
    thrust::device_vector<int> ind_device(countOnes * Nparticles);
    
    long long send_start = get_time();
    
    thrust::device_vector<unsigned char> I_device(I, I + (IszX * IszY * Nfr));
    thrust::device_vector<int> objxy_device = objxy;
    thrust::device_vector<double> xj_device = xj;
    thrust::device_vector<double> yj_device = yj;
    thrust::device_vector<int> seed_device(seed, seed + Nparticles);

    long long send_end = get_time();
    printf("TIME TO SEND TO GPU: %f\n", elapsed_time(send_start, send_end));

    double *arrayX_GPU = (double *) thrust::raw_pointer_cast(&*(arrayX_device.begin()));
    double *arrayY_GPU = (double *) thrust::raw_pointer_cast(&*(arrayY_device.begin()));
    double *CDF_GPU = (double *) thrust::raw_pointer_cast(&*(CDF_device.begin()));
    double *u_GPU = (double *) thrust::raw_pointer_cast(&*(u_device.begin()));
    double *likelihood_GPU = (double *) thrust::raw_pointer_cast(&*(likelihood_device.begin()));
    double *weights_GPU = (double *) thrust::raw_pointer_cast(&*(weights_device.begin()));
    double *partial_sums = (double *) thrust::raw_pointer_cast(&*(partial_sums_device.begin()));
    double *xj_GPU = (double *) thrust::raw_pointer_cast(&*(xj_device.begin()));
    double *yj_GPU = (double *) thrust::raw_pointer_cast(&*(yj_device.begin()));

    int *ind_GPU = (int *) thrust::raw_pointer_cast(&*(ind_device.begin()));
    int *objxy_GPU = (int *) thrust::raw_pointer_cast(&*(objxy_device.begin()));
    int *seed_GPU = (int *) thrust::raw_pointer_cast(&*(seed_device.begin()));

    unsigned char *I_GPU = (unsigned char *) thrust::raw_pointer_cast(&*(I_device.begin()));
    ...
    arrayX = arrayX_device;
    long long arrayX_time = get_time();

    arrayY = arrayY_device;
...
int main(int argc, char * argv[]) {
    ...
    int i;
    for (i = 0; i < Nparticles; i++) seed[i] = i;
        // seed[i] = time(0) * i;
