...
extern "C" void render(int width, int height, float tx, float ty, float scale, float cx, float cy,
                       dim3 blockSize, dim3 gridSize, eFilterMode filter_mode, uchar4 *output,thrust::block_2d<uchar4> &block_d_output);
...
void runAutoTest(int argc, char **argv, const char *dump_filename, eFilterMode filter_mode)
{
    ...
    render(imageWidth, imageHeight,
           tx, ty, scale, cx, cy,
           blockSize, gridSize, filter_mode, d_output,block_d_output);
