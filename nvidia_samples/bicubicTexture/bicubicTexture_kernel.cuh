...
class d_render_functor
{

public:
  float tx,  ty,  scale, cx, cy;
  d_render_functor(float tx, float ty, float scale, float cx, float cy)
  {
    this->tx = tx;
    this->ty = ty;
    this->cx = cx;
  ...
  {
    float u = (window.start_x-cx)*scale+cx + tx;
    float v = (window.start_y-cy)*scale+cy + ty;
    float c = tex2D(tex, u, v);
    window[0][0]= make_uchar4(c * 0xff, c * 0xff, c * 0xff, 0);

  }
};
class d_renderFastBicubic_functor
{

public:
  float tx,  ty,  scale, cx, cy;
  d_renderFastBicubic_functor(float tx, float ty, float scale, float cx, float cy)
  {
    this->tx = tx;
    this->ty = ty;
    this->cx = cx;
...
class d_renderBicubic_functor
{

public:
  float tx,  ty,  scale, cx, cy;
  d_renderBicubic_functor(float tx, float ty, float scale, float cx, float cy)
  {
    this->tx = tx;
    this->ty = ty;
    this->cx = cx;
...
class d_renderCatrom_functor
{

public:
  float tx,  ty,  scale, cx, cy;
  d_renderCatrom_functor(float tx, float ty, float scale, float cx, float cy)
  {
    this->tx = tx;
    this->ty = ty;
    this->cx = cx;
