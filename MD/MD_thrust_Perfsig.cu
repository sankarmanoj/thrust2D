...
{
    float sum = thrust::transform_reduce(vel_gpuX.begin(),vel_gpuX.end(),squareOp(),0,thrust::plus<float>())
              + thrust::transform_reduce(vel_gpuY.begin(),vel_gpuY.end(),squareOp(),0,thrust::plus<float>())
              + thrust::transform_reduce(vel_gpuZ.begin(),vel_gpuZ.end(),squareOp(),0,thrust::plus<float>());
	return 0.5 * mass * sum;
}
