...
thrust::device_vector<float> accnew_gpu;
thrust::device_vector<float> velnew_gpu;
...
thrust::device_vector<float> parvel_gpu;
thrust::device_vector<float> acc_gpu;
thrust::device_vector<float> force_gpu;

thrust::device_vector<float> pos_gpuX;
thrust::device_vector<float> pos_gpuY;
thrust::device_vector<float> pos_gpuZ;

thrust::device_vector<float> vel_gpu;
...
{
	accnew_gpu = *(new thrust::device_vector<float>(nd*np));
	velnew_gpu = *(new thrust::device_vector<float>(nd*np));
  ...
  parvel_gpu = *(new thrust::device_vector<float>(nd*np));
	acc_gpu = *(new thrust::device_vector<float>(nd*np));
	force_gpu = *(new thrust::device_vector<float>(nd*np));
  ...
  vel_gpu = *(new thrust::device_vector<float>(nd*np));
}

extern "C"
void copyDataToGPU(float *h_acc, float *h_force, float *h_vel, float *h_pos, int nd, int np)
{
	acc_gpu.assign( h_acc ,h_acc + nd*np);
	force_gpu.assign( h_force ,h_force + nd*np);
	vel_gpu.assign( h_vel ,h_vel + nd*np);
...
public:
  compute_forceonparticle_functor(float PI2,  float currentposx, float currentposy, float currentposz)
  {
    this->PI2 = PI2;
  ...
  __device__ float operator() (Tuple input, Tuple output)
  {
    float px = thrust::get<0>(input);
    float py = thrust::get<1>(input);
    float pz = thrust::get<2>(input);
    float dist = (this->currentposx - px)*(this->currentposx - px) + (this->currentposy - py)*(this->currentposy - py) + (this->currentposz - pz)*(this->currentposz - pz) ;
    dist = sqrt(dist);
    float dist2 = (dist2 < PI2) ? dist2 : PI2;
    if(dist<0.0001)
    {
...
thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(pos_gpuX.begin(),pos_gpuY.begin(),pos_gpuZ.begin())),
                  thrust::make_zip_iterator(thrust::make_tuple(pos_gpuX.end(),pos_gpuY.end(),pos_gpuZ.end())),
                  thrust::make_zip_iterator(thrust::make_tuple(parforce_gpuX.begin(),parforce_gpuY.begin(),parforce_gpuZ.begin())),
                  parpot_gpu.begin(),compute_forceonparticle_functor(PI2,pos_gpuX[currentMoleculeIndex],pos_gpuY[currentMoleculeIndex],pos_gpuZ[currentMoleculeIndex]));
