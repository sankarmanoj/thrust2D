...
namespace thrust
{
  ...
  void binary_transform_kernel (window_iterator<T> *input1,window_iterator<T> * input2, window_iterator<T> * output, warp_launcher_config mConfiguration, Func f)
  {
    ...
    if(mConfiguration.block_dim_x<=(mConfiguration.stride_x/2)+mConfiguration.stride_x*(blockIdx.x*blockDim.x+ threadIdx.x))
      return;
    __syncthreads();
    window_2d<T> shared_window1(input1->b,shared_memory,blockIdx.x*mConfiguration.warp_size + threadIdx.x,blockIdx.y*mConfiguration.warp_size + threadIdx.y,threadIdx.x,threadIdx.y,input1->window_dim_x,input1->window_dim_y,mConfiguration.shared_size_x,mConfiguration.warp_size+mConfiguration.padding);
    window_2d<T> shared_window2(input2->b,shared_memory+mConfiguration.shared_size_x,blockIdx.x*mConfiguration.warp_size + threadIdx.x,blockIdx.y*mConfiguration.warp_size + threadIdx.y,threadIdx.x,threadIdx.y,input2->window_dim_x,input2->window_dim_y,mConfiguration.shared_size_x,mConfiguration.warp_size+mConfiguration.padding);
