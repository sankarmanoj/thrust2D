...
namespace thrust
{
  ...
  void transform_kernel (window_iterator<T> *input, window_iterator<T> * output, warp_launcher_config mConfiguration, Func f)
  {
    ...
    __syncthreads();
    if((threadIdx.x%mConfiguration.sx)||(threadIdx.y%mConfiguration.sy))
      return;
    {
        window_2d<T> shared_window(shared_memory,blockIdx.x*32 + threadIdx.x,blockIdx.y*32 + threadIdx.y,threadIdx.x,threadIdx.y,input->window_dim_x,input->window_dim_y,32,32);
  ...
  void transform(cuda::shared_policy,Iterator begin1, Iterator end1, Iterator begin2, Func f)
  {
    ...
    mConfiguration.sx = begin1.stride_x;
    mConfiguration.sy = begin1.stride_y;
  ...
  void for_each_kernel (window_iterator<T> *input, warp_launcher_config mConfiguration, Func f)
  {
    ...
    __syncthreads();
    (*(input->b))[make_int2(blockIdx.x*32 + threadIdx.x,blockIdx.y*32 + threadIdx.y)]=shared_memory[threadIdx.y*32+threadIdx.x];
  ...
  void for_each(cuda::shared_policy,Iterator begin1, Iterator end1, Func f)
  {
    ...
    cudaMalloc((void **)&device_begin_1, sizeof(Iterator));
    cudaMemcpy(device_begin_1,&begin1,sizeof(Iterator),cudaMemcpyHostToDevice);
    for_each_kernel<<<dim3(begin1.block_dim_x/32,begin1.block_dim_y/32,1),dim3(32,32,1),size_along_y*size_along_x*sizeof(T)>>>(device_begin_1,mConfiguration,f);
  }
}
