...
public:
	...
	SRADFunctor1N (int cols, int rows, float q0sqr)
	{
		this->cols = cols;
		this->rows = rows;
		this->q0sqr = q0sqr;
...
public:
	...
	SRADFunctor2N (int cols,int rows,float lambda,float q0sqr)
	{
		this->cols = cols;
		this->rows = rows;
		this->lambda = lambda;
	...
	__device__ float operator() ( const thrust::window_2d<float> &c,const thrust::window_2d<float> &w) const
	{
		int ty = w.window_dim_y/2;
		int tx = w.window_dim_x/2;
		int N = ty-1;
