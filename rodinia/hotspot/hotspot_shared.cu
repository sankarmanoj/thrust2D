...
public:
	...
	__device__ void operator() (const thrust::window_2d<float> &w,const thrust::window_2d<float> &p ) const
	{
		...
		{
			w[ty][tx] =  w[ty][tx] + stepDivCap * (p[ty][tx] + \
	...
	void run(int argc, char** argv)
	{
		...
		for (t = 0; t < total_iterations; t+=pyramid_height)
		{
			...
			thrust::transform(thrust::cuda::shared,wv.begin(),wv.end(),wp.begin(),functor);
		}
