...
int main(int argc, char *argv []){
  ...
  printf("WG size of kernel = %d \n", NUMBER_THREADS);
	...
	int ret = open_input_file(video_file_name);
	printf("Input File Opened - %u\n",ret);
	ret = open_output_file("output.avi");
	printf("Output File Opened - %u\n",ret);
	AVFormatContext *temp_ifmt_ctx = ifmt_ctx;
	avformat_close_input(&temp_ifmt_ctx);
	printf("FFMPEG Input File Closed\n");
	// open movie file
	...
	// common
	common.in_rows = common.tSize + 1 + common.tSize; //in_rows = 51

	common.in_cols = common.in_rows;	//in_rows = 51
	common.in_elem = common.in_rows * common.in_cols; //in_elem = 2601
	...
	for(common_change.frame_no=0; common_change.frame_no<frames_processed; common_change.frame_no++){
		...
		AVPixelFormat pixelFormat = 	av_get_pix_fmt ("pal8");
		int size = avpicture_get_size(pixelFormat,common.frame_cols,common.frame_rows);
		 uint8_t *picture_buf;
		  picture_buf =(uint8_t *) av_malloc(size);
			avpicture_fill((AVPicture *)output_frame, picture_buf, pixelFormat, common.frame_cols,common.frame_rows);
		// output_frame = alloc_picture(pixelFormat,common.frame_cols,common.frame_rows);
		printf("Size required per frame = %d\n",size);
...
// // 			thrust::for_each(mCount,mCount + common.conv_elem,kernelConvul(&d_in_mod_temp_block));
		...
		for (int i = 0; i<common.frame_elem; i++)
			output_frame->data[0][i] = floor(frame[i]);
		output_frame->width = common.frame_cols;
		output_frame->height = common.frame_rows;
		output_frame->format = (int)pixelFormat;
		ret = enc_func(ofmt_ctx->streams[stream_index]->codec, &enc_pkt,
						output_frame, &got_frame);
		// printf("enc_func return = %d\n",ret);
